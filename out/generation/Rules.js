...
class Rules {
    static determineNextGeneration(currentGeneration) {
        ...
        const nextGeneration = new Array(numRows).fill(null).map(() => new Array(numCols).fill(false));
     ...
     */
    static willSpawn(generation, x, y) {
     ...
     */
    static willStayAlive(generation, x, y) {
        const livingNeighbors = Rules.countLivingNeighbors(generation, x, y);
        return generation[x][y] && (livingNeighbors === 2 || livingNeighbors === 3);
    }
    static countLivingNeighbors(generation, x, y) {
